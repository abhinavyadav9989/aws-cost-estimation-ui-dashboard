...
interface ServiceRow {
  key: string;
  name: string;
  baseline: number;      // USD / month (current)
  elasticity: number;    // 0..1 (portion that scales with users)
  notes?: string;
}

// Friendly helper type for CSV building
interface RowCalc extends ServiceRow { projected?: number }
...
const PRICING_LINKS: Record<string, { label: string; href: string }[] > = {
...
export function applyFx(amountUSD: number, currency: Currency, fxRateINR: number) {
  if (currency === "INR") {
...
export function fmtAmount(amountUSD: number, currency: Currency, fxRateINR: number, opts: Intl.NumberFormatOptions = { maximumFractionDigits: 2 }) {
...
export function computeProjected(baseline: number, elasticity: number, factor: number, trafficBoost: number = 1): number {
  const fixed = baseline * (1 - elasticity);
  const variable = baseline * elasticity * factor * trafficBoost;
  return fixed + variable;
}

export function buildCSV(rows: RowCalc[], baselineTotal: number, projectedTotal: number): string {
  const header = ["Service","BaselineUSD","Elasticity","ProjectedUSD"].join(",");
  const body = rows.map((r) => [r.name, r.baseline.toFixed(2), r.elasticity.toFixed(2), (r as any).projected?.toFixed(2) ?? "0.00"].join(","));
...
function TL({ label, term }: { label: string; term: string }) {
  return (
...
// Link out (styled)
function AwsLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
...
export default function AWSBudgetEstimator() {
  ...
  const [currentUsers, setCurrentUsers] = useState<number>(50000);
  const [futureUsers, setFutureUsers]   = useState<number>(100000);
  const [monthlyGrowthPct, setMonthlyGrowthPct] = useState<number>(10);
  ...
  const [fxRateINR, setFxRateINR] = useState<number>(85); // editable, display-only conversion

  // Usage heaviness (maps to traffic multiplier under the hood)
  const [usageWeight, setUsageWeight] = useState<"light" | "medium" | "heavy">("medium");
  ...
  const projectCost = (factor: number) => {
    return services.map((s) => {
      ...
      return { ...s, projected: total } as RowCalc & { projected: number };
    });
  };
  ...
  const projectedTotalUSD = useMemo(() => nowRows.reduce((acc, r) => acc + (r as any).projected, 0), [nowRows]);
  ...
  const forecastUSD = useMemo(() => {
    ...
    for (let m = 0; m <= months; m++) {
      ...
      const total = rows.reduce((acc, r) => acc + (r as any).projected, 0);
      data.push({ month: `M${m}`, users: round ? Math.round(users) : users, total, ...Object.fromEntries(rows.map(r => [r.key, (r as any).projected])) });
      users = users * (1 + monthlyGrowthPct / 100);
    }
    return data;
  }, [currentUsers, months, monthlyGrowthPct, services, trafficMultiplier, round]);

  const pieDataUSD = useMemo(() => nowRows.map((r) => ({ name: r.name, value: (r as any).projected })), [nowRows]);

  const resetDefaults = () => setServices(DEFAULT_SERVICES.map(s => ({...s})));

  const downloadCSV = () => {
    const csv = buildCSV(nowRows as RowCalc[], baselineTotalUSD, projectedTotalUSD);
  ...
  // Local formatters
  const fmtLocal = (usd: number, opts: Intl.NumberFormatOptions = { maximumFractionDigits: 2 }) => fmtAmount(usd, currency, fxRateINR, opts);
  const toLocal = (usd: number) => applyFx(usd, currency, fxRateINR);
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <ReTooltip formatter={(val: any) => fmtLocal(Number(val))} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
                          ...
                          <tbody>
                            {nowRows.map((r: any, idx: number) => (
                              <tr key={r.key} className="border-b last:border-0">
                                ...
                                <td className="py-2 pr-4 font-semibold">{fmtLocal(r.projected as number)}</td>
                    ...
                    <div className="h-80">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={forecastUSD} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                          ...
                          <ReTooltip formatter={(val: any, name: string) => name === "total" ? fmtLocal(Number(val)) : Number(val).toFixed(0)} />
                          <Legend />
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {DEFAULT_SERVICES.map((s) => (
                      <div key={s.key} className="rounded-xl border p-3">
                        ...
                        <div className="text-sm text-neutral-700">{TERMS[s.key as keyof typeof TERMS] || s.notes || ""}</div>
...
try {
  ...
  if (typeof window !== "undefined" && !(window as any).__AWS_COST_ESTIMATOR_TESTED__) {
    (window as any).__AWS_COST_ESTIMATOR_TESTED__ = true;
